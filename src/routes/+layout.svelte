<script>
	...
	const navItems = [
		...
		{ href: '/metrics', label: 'Sensors', icon: 'metrics' },
...
<nav aria-label="Mobile navigation" class="btm-nav visible lg:hidden z-50 border-t border-green-500/20 bg-black/90 backdrop-blur-sm">
	{#each navItems as item}
		...
		<a
			href={item.href}
			class="border-transparent {active ? 'text-cyan-400' : 'text-green-500/60'}"
			class:active
		...
		>
			...
			{/if}
			<span class="btm-nav-label text-[10px] font-mono tracking-wider">{item.label}</span>
		</a>
	{/each}
</nav>
