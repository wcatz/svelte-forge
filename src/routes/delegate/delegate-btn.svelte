...
<div class="relative">
	<ActionBtn action={toggleShow} type="button" text="Delegate" {wait} {status} />
	<div class="absolute left-12 z-10 mt-5 flex -translate-x-1/2 px-4 {show ? '' : 'hidden'}">
		<div
			on:click|stopPropagation
			class="flex-auto overflow-hidden rounded-3xl bg-black text-sm leading-6 shadow-lg ring-1 ring-gray-900/5"
		>
			<div class="p-4">
				<div
					on:click|stopPropagation={() => delegate('nami')}
					class="group relative flex rounded-lg p-2 hover:bg-gray-600 cursor-pointer"
				>
					<div
						class="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-800 p-2"
					>
						<NamiIcon />
					</div>
					<div class="font-bold text-white flex items-center pl-4 pr-2">Nami</div>
				</div>
				<div
					on:click|stopPropagation={() => delegate('eternl')}
					class="group relative flex rounded-lg p-2 hover:bg-gray-600 cursor-pointer"
				>
					<div
						class="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-800 p-2"
					...
					<div class="font-bold text-white flex items-center pl-4 pr-2">Eternl</div>
				</div>
				<div
					on:click|stopPropagation={() => delegate('flint')}
					class="group relative flex rounded-lg p-2 hover:bg-gray-600 cursor-pointer"
				>
					<div
						class="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-800 p-2"
					...
					<div class="font-bold text-white flex items-center pl-4 pr-2">Flint</div>
				</div>
				<div
					on:click|stopPropagation={() => delegate('yoroi')}
					class="group relative flex rounded-lg p-2 hover:bg-gray-600 cursor-pointer"
				>
					<div
						class="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-800 p-2"
					...
					<div class="font-bold text-white flex items-center pl-4 pr-2">Yoroi</div>
				</div>
				<div
					on:click|stopPropagation={() => delegate('gerowallet')}
					class="group relative flex rounded-lg p-2 hover:bg-gray-600 cursor-pointer"
				>
					<div
						class="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-800 p-2"
					...
					<div class="font-bold text-white flex items-center pl-4 pr-2">Gero</div>
				</div>
				<div
					on:click|stopPropagation={() => delegate('lace')}
					class="group relative flex rounded-lg p-2 hover:bg-gray-600 cursor-pointer"
				>
					<div
						class="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-800 p-2"
					...
					<div class="font-bold text-white flex items-center pl-4 pr-2">Lace</div>
				</div>
			</div>
		</div>
	</div>
...
<Modal bind:this={errorModal} hideAction={true} outClick={true} callback={stopWait}>
	...
	<p slot="body">Oops, something unexpected happened. Please try again later or contact support.</p>
</Modal>

<Modal bind:this={connectorErrorModal} hideAction={true} outClick={true} callback={stopWait}>
	<svelte:fragment slot="title"
		><span class="text-error">Wallet extension not found!</span></svelte:fragment
	>
	<p slot="body">Make sure the selected wallet extension is installed.</p>
</Modal>
...
<Modal bind:this={networkErrorModal} hideAction={true} outClick={true} callback={stopWait}>
	...
	<p slot="body">Make sure the selected wallet is set to mainnet.</p>
</Modal>
...
<Modal bind:this={insufficientFundsModal} hideAction={true} outClick={true} callback={stopWait}>
	...
	>
	<p slot="body">Make sure the selected account have sufficient funds.</p>
</Modal>

<Modal bind:this={alreadyDelegatedErrorModal} hideAction={true} outClick={true} callback={stopWait}>
	<svelte:fragment slot="title"
		><span class="text-success">Account already delegated to OTG!</span></svelte:fragment
	>
	<p slot="body">The selected wallet account is already delegated to Star Forge [OTG].</p>
</Modal>

<Modal bind:this={txSentModal} hideAction={true} outClick={false}>
	<svelte:fragment slot="title">Delegation transaction sent!</svelte:fragment>
	<div slot="body">
...
<Modal bind:this={successModal} hideAction={true} outClick={true} callback={stopWait}>
	<svelte:fragment slot="title">Delegation Active!</svelte:fragment>
	<div slot="body">
