...
<div class="relative">
  ...
  <div class="absolute left-12 z-10 mt-5 flex -translate-x-1/2 px-4 {show ? '' : 'hidden'}">
    <div on:click|stopPropagation class="flex-auto overflow-hidden rounded-3xl bg-black text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
      <div class="p-4">
...
<Modal bind:this="{alreadyDelegatedErrorModal}" hideAction="{true}" outClick="{true}" callback="{stopWait}">
  <svelte:fragment slot="title"><span class="text-error">Account already delegated to OTG!</span></svelte:fragment>
  <p slot="body">
